...
def do_work(url):
    try:
        isbns = find_isbns(url)
        print('isbns', isbns)
        query = query_book('', isbns, skip_shbi=True)
        return isbns, url, query
...
def write_books_data(books, file_suffix, out_dir, temp_csv='temp.csv'):
    columns = ['ISBNs', 'URL', 'BookChor', 'BookChor:ISBNS']
    with open(temp_csv, 'w') as fw:
        ...
        for title in books:
            ...
            row = (title, ', '.join(book[0]), book[1], bchor, ', '.join(book[-1]['bookchor']))
            writer.writerow(row)
